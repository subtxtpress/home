...
A single-file OSINT tool for monitoring thermal anomalies, infrastructure impact, conflict events, satellite imagery, maritime boundaries, and live aircraft tracking across the Middle East theater. Built for investigative journalists, OSINT analysts, and GIS researchers.
...
- **Conflict Events**: Triple-source conflict intelligence: live GDELT Global Knowledge Graph data (armed conflict, terror/explosions, violence, protests, military activity), ACLED expert-coded events (human-verified with actor details, fatality counts, and event sub-types; rolling 30-day window, subject to account-tier access limits), and UCDP GED verified archive (through Dec 2024 with fatality estimates and conflict classifications). Switchable source tabs with event-type filtering and temporal alignment to thermal detections.
...
- **Live data**: Pulls directly from NASA FIRMS, GDELT, ACLED, and UCDP, routed through three self-built Supabase edge functions (`gdelt-proxy`, `acled-proxy`, `ucdp-proxy`) to handle CORS restrictions and, for ACLED, a full OAuth login/refresh cycle server-side. No manual setup or account required to use the tool itself.
...
- **Satellite imagery**: Three toggleable optical imagery layers (MODIS, Sentinel-2, Maxar) for visual corroboration of thermal detections. All free and open, no API keys needed on the user's end.
...
The frontend is a single HTML file (~5,000 lines), loaded with no build step. It is backed by three self-built Supabase edge functions (`gdelt-proxy`, `acled-proxy`, `ucdp-proxy`) that proxy live requests around browser CORS restrictions and, for ACLED, handle the OAuth login and token-refresh cycle server-side. It is not a purely client-side application; the backend functions are a real, necessary part of how the live data layers work.

**Stack:** MapLibre GL JS v4.1.2 / Turf.js v7 / IBM Plex typography / CartoDB + Esri basemaps / Supabase Edge Functions (Deno)
...
| Conflict (Coded) | [ACLED](https://acleddata.com/) | Expert-coded, ~1-2 week lag, rolling 30-day window; recency further limited by account access tier |
...
- **Conflict Events**: GDELT (live), ACLED (expert-coded, tier-limited recency), and UCDP GED (archive) data showing armed conflict, protests, and military activity
